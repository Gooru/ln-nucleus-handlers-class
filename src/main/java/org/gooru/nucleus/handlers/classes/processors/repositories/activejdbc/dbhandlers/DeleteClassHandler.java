...
class DeleteClassHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    LazyList<AJEntityClass> entityClasses = AJEntityClass
        .findBySQL(AJEntityClass.DELETE_QUERY, context.classId());
    ...
    if (!this.entityClass.isCurrentVersion() || this.entityClass.isArchived()) {
      LOGGER.warn("Class '{}' is either archived or not of current version, can not delete",
          context.classId());
      return new ExecutionResult<>(
          MessageResponseFactory
    ...
    if (courseId != null && !courseId.isEmpty()) {
      LOGGER.warn("Delete request for class '{}' which is having a course '{}'", context.classId(),
          courseId);
    }
    ...
    try {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createNoContentResponse(RESOURCE_BUNDLE.getString("deleted"),
              EventBuilderFactory.getDeleteClassEventBuilder(context.classId())),
          ExecutionResult.ExecutionStatus.SUCCESSFUL);
    } catch (DBException dbe) {
      LOGGER.warn("Unable to delete membership details for class '{}' delete request",
          context.classId(), dbe);
      return new ExecutionResult<>(MessageResponseFactory.createInternalErrorResponse(
          RESOURCE_BUNDLE.getString("membership.delete.failure")),
          ExecutionResult.ExecutionStatus.FAILED);
    }
  }
