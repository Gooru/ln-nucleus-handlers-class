...
class RemoveInviteHandler implements DBHandler {

  private static final Logger LOGGER = LoggerFactory.getLogger(RemoveInviteHandler.class);
  private static final ResourceBundle RESOURCE_BUNDLE = ResourceBundle.getBundle("messages");
  private final ProcessorContext context;

  RemoveInviteHandler(ProcessorContext context) {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    ...
    if (context.userId() == null || context.userId().isEmpty()
      ...
      LOGGER.warn("Anonymous user attempting to remove invitation from class");
      return new ExecutionResult<>(
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    return AuthorizerBuilder.buildRemoveInviteAuthorizer(this.context).authorize(entityClass);
  }

  @Override
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      AJClassMember.delete(AJClassMember.DELETE_INVITE_QUERY_FILTER, context.classId(),
          context.studentEmail());
      return new ExecutionResult<>(
          MessageResponseFactory
              .createNoContentResponse(RESOURCE_BUNDLE.getString("invite.removed"),
                  EventBuilderFactory
                      .getInviteRemovalEventBuilder(context.classId(), context.studentEmail())),
          ExecutionResult.ExecutionStatus.SUCCESSFUL);

    } catch (DBException dbe) {
      LOGGER.error("Error trying to remove invitations", dbe);
      return new ExecutionResult<>(
          MessageResponseFactory
