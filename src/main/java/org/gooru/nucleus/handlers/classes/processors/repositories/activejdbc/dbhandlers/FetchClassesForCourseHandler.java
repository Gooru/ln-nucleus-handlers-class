    ...
    if (context.courseId() == null || context.courseId().isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInvalidRequestResponse(RESOURCE_BUNDLE.getString("missing.course")),
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      LazyList<AJEntityClass> classes =
          AJEntityClass.where(AJEntityClass.FETCH_FOR_COURSE_QUERY_FILTER, context.courseId());
      JsonArray classesList = new JsonArray(JsonFormatterBuilder
          .buildSimpleJsonFormatter(false, AJEntityClass.FETCH_QUERY_FIELD_LIST).toJson(classes));
      return new ExecutionResult<>(
          MessageResponseFactory
              .createOkayResponse(new JsonObject().put(RESPONSE_BUCKET_CLASSES, classesList)),
