...
public class ArchiveClassHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> validateRequest() {
        LazyList<AJEntityClass> entityClasses = AJEntityClass.findBySQL(AJEntityClass.ARCHIVE_QUERY, context.classId());
        if (entityClasses.isEmpty()) {
            LOGGER.warn("Class id '{}' not present in DB", context.classId());
            return new ExecutionResult<>(MessageResponseFactory.createNotFoundResponse(
                RESOURCE_BUNDLE.getString("class.id") + context.classId()), ExecutionResult.ExecutionStatus.FAILED);
        }
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        ...
        if (this.entityClass.isArchived()) {
            LOGGER.warn("Class '{}' is already archive, sending success without event", context.classId());
            return new ExecutionResult<>(
        ...
        if (!this.entityClass.isCurrentVersion()) {
            ...
            return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                RESOURCE_BUNDLE.getString("class.incorrect.version")), ExecutionResult.ExecutionStatus.FAILED);
        }
