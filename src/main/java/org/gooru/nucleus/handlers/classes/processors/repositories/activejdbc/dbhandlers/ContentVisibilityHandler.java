...
public class ContentVisibilityHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    // Check authorization
    ExecutionResult<MessageResponse> result = AuthorizerBuilder
        .buildContentVisibilityAuthorizer(this.context).authorize(entityClass);
    if (result.hasFailed()) {
      return result;
    }
    ...
    if (courseId == null) {
      LOGGER
          .error("Class '{}' is not assigned to course, hence cannot set content visibility",
              context.classId());
      return new ExecutionResult<>(
          MessageResponseFactory
    ...
    return ContentVisibilityHelper
        .validatePayloadWithDB(context.request(), courseId, this.context.classId());
  }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    try {
      LazyList<AJEntityCollection> collections = AJEntityCollection
          .findBySQL(AJEntityCollection.FETCH_COLLECTIONS_CLASS_VISIBILITY_BY_ID, courseId,
      ...
      return new ExecutionResult<>(MessageResponseFactory
          .createNoContentResponse(RESOURCE_BUNDLE.getString("updated"),
              EventBuilderFactory
                  .getContentVisibleEventBuilder(context.classId(), context.request())),
  ...
  private Map<String, String> getInputToMark() {
    ...
    JsonArray contentsCollection = this.context.request()
        .getJsonArray(AJEntityClass.CV_COLLECTIONS);
    JsonArray contentsAssessment = this.context.request()
        .getJsonArray((AJEntityClass.CV_ASSESSMENTS));
    ...
    input.forEach(item -> {
      ...
      collectionVisibilityMap.put(itemJson.getString(AJEntityCollection.ID),
          itemJson.getString(AJEntityCollection.JSON_KEY_VISIBLE));
    });
