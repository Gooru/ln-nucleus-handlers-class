...
class FetchClassesForUserHandler implements DBHandler {
  ...
  private ExecutionResult<MessageResponse> populateOwnedOrCollaboratedClassesId(JsonObject result) {
    try {
      LazyList<AJEntityClass> classes = AJEntityClass
          .findBySQL(AJEntityClass.FETCH_FOR_OWNER_COLLABORATOR_QUERY,
      ...
      LOGGER.warn("Unable to fetch owned or collaborated classes for user '{}'", context.userId(),
          dbe);
      return new ExecutionResult<>(
  ...
  private ExecutionResult<MessageResponse> populateMembershipClassesId(JsonObject result) {
    try {
      ...
      List memberClassIdList = Base
          .firstColumn(AJClassMember.FETCH_USER_MEMBERSHIP_QUERY, context.userId());
    ...
    classes.forEach(classEntry -> {
      JsonObject classJson = new JsonObject(
          JsonFormatterBuilder.buildSimpleJsonFormatter(false, AJEntityClass.FETCH_QUERY_FIELD_LIST)
              .toJson(classEntry));
      String courseTitle = courseTitleMap
          .getOrDefault(classEntry.getString(AJEntityClass.COURSE_ID), null);
      String courseVersion = courseVersionMap
          .getOrDefault(classEntry.getString(AJEntityClass.COURSE_ID), null);
  ...
  private static void populateCourseTitleVersionForIds(List<String> courseIdList,
      Map<String, String> courseTitleMap,
      Map<String, String> courseVersionMap) {
    LazyList<AJEntityCourse> courses = AJEntityCourse
        .findBySQL(AJEntityCourse.SELECT_COURSE_TITLE_VERSION,
            Utils.convertListToPostgresArrayStringRepresentation(courseIdList));

    courses.forEach(course -> {
      courseTitleMap
          .put(course.getString(AJEntityCourse.ID), course.getString(AJEntityCourse.TITLE));
      courseVersionMap
          .put(course.getString(AJEntityCourse.ID), course.getString(AJEntityCourse.VERSION));
    });
  }
  ...
  private ExecutionResult<MessageResponse> populateTeacherDetails(JsonObject result) {
    try {
      ...
      LOGGER
          .warn("Unable to fetch teacher details for classes for user '{}'", context.userId(), dbe);
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInternalErrorResponse(RESOURCE_BUNDLE.getString("error.from.store")),
          ExecutionResult.ExecutionStatus.FAILED);
