...
class CreateClassHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    return new ExecutionResult<>(
        MessageResponseFactory.createCreatedResponse(this.entityClass.getId().toString(),
            EventBuilderFactory
                .getCreateClassEventBuilder(this.entityClass.getString(AJEntityClass.ID))),
  ...
  private void autoPopulate() {
    // Need to populate modifier id, creator id, version. Note that "code"
    // is already done at higher level to enable exception handling
  ...
  private boolean checkUniqueness(String resultCode) {
    try {
      ...
      LOGGER.error("Error checking unique code for user '{user}' and code '{}'", context.userId(),
          resultCode, e);
    }
    return false;
  }
