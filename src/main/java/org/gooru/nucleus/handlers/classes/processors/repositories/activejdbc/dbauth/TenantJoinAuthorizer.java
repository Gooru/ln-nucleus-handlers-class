 ...
 */
public class TenantJoinAuthorizer implements Authorizer<AJEntityClass> {

  private final ProcessorContext context;
  private static final ResourceBundle RESOURCE_BUNDLE = ResourceBundle.getBundle("messages");
  private static final Logger LOGGER = LoggerFactory.getLogger(TenantJoinAuthorizer.class);

  public TenantJoinAuthorizer(ProcessorContext context) {
  ...
  public ExecutionResult<MessageResponse> authorize(AJEntityClass model) {
    ...
    ClassTenantAuthorization authorization = ClassTenantAuthorizationBuilder
        .build(classTenantTree, userTenantTree,
            ClassAttributes.build(model.isPublished()));

    if (authorization.canJoin()) {
      return new ExecutionResult<>(null, ExecutionResult.ExecutionStatus.CONTINUE_PROCESSING);
    }
    LOGGER.warn("Tenant auth failed for class join for user {} for class {}", context.userId(),
        model.getId());
