...
public final class ContentVisibilityHelper {
  ...
  /**
   * Validate that all the collections and assessments are present and not deleted and are not
   * visible for that class. In case they are either not present, or deleted or already visible,
   * this is an error. Note that this function does not check the class setting. This should have
   * happened already.
   *
   * @return ExecutionResult which signifies whether to continue or not
   */
  public static ExecutionResult<MessageResponse> validatePayloadWithDB(JsonObject payload,
      String courseId,
      String classId) {
    ...
    return validateCollectionsAssessmentsExistence(contentsCollection, contentsAssessment, courseId,
        classId);
  }

  /**
   * Validate that based on class setting, the payload is doing right thing. If class setting is
   * visible all then it is an error to call this API If collections are visible as per class
   * setting, then it is error to try and make collections visible If all is hidden, then call is
   * assumed to be fine.
   *
    ...
    if (AJEntityClass.CONTENT_VISIBILITY_TYPE_VISIBLE_ALL.equalsIgnoreCase(visibility)) {
      return new ExecutionResult<>(MessageResponseFactory
          .createInvalidRequestResponse(
              RESOURCE_BUNDLE.getString("class.setting.visible.all.error")),
  ...
  private static ExecutionResult<MessageResponse> validateCollectionsAssessmentsExistence(
    ...
    if (!input.isEmpty()) {
      try {
        ...
        long count =
            Base.count(AJEntityCollection.TABLE_COLLECTION,
                AJEntityCollection.COLLECTIONS_QUERY_FILTER,
        ...
        LOGGER
            .error("Error validating collection counts for visibility setting for course '{}'",
                courseId, dbe);
