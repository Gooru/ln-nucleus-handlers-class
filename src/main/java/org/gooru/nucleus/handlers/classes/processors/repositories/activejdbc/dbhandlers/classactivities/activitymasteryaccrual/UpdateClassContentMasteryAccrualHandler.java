package org.gooru.nucleus.handlers.classes.processors.repositories.activejdbc.dbhandlers.classactivities.activitymasteryaccrual;
...
public class UpdateClassContentMasteryAccrualHandler implements DBHandler {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(UpdateClassContentMasteryAccrualHandler.class);
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    try {
      ...
      this.allowMasteryAccrual = this.context.request()
          .getBoolean(AJEntityClassContents.ALLOW_MASTERY_ACCRUAL);
    } catch (MessageResponseWrapperException mrwe) {
      return new ExecutionResult<>(mrwe.getMessageResponse(),
          ExecutionResult.ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    return new ExecutionResult<>(
        MessageResponseFactory.createNoContentResponse(RESOURCE_BUNDLE.getString("updated"),
            EventBuilderFactory.getClassContentMasteryAccrualEventBuilder(classContents.getId(),
  ...
  private void validateContextRequestFields() {
    JsonObject errors = new DefaultPayloadValidator().validatePayload(context.request(),
        AJEntityClassContents.updateMasteryAccrualFieldSelector(),
  ...
  private void validateAndInitializeClassContents() {
    ...
    if (!FieldValidator
        .validateDateWithFormat(dcaAddedDate, DateTimeFormatter.ISO_LOCAL_DATE, false, true)) {
      LOGGER.warn("Content {} either not scheduled or it's scheduled for past class activity {}",
          contentId, context.classId());
      throw new MessageResponseWrapperException(
          MessageResponseFactory.createInvalidRequestResponse(
              RESOURCE_BUNDLE.getString("mastery.accrual.not.allowed")));
