...
public final class EventBuilderFactory {
  ...
  private static final String EVT_CLASS_CONTENT_MEETING_CANCEL = "event.class.content.meetingcancel";
  ...
  public static EventBuilder getAddStudentsToClassEventBuilder(String classId, JsonArray students) {
    ...
    students.forEach(student -> {
      eventsArray.add(new JsonObject().put(EVENT_NAME, EVT_CLASS_STUDENT_JOIN).put(EVENT_BODY,
        new JsonObject().put(CLASS_ID, classId).put(STUDENT_ID, student)));
    });
    return () -> new JsonObject().put("events", eventsArray);
  }
  ...
  public static EventBuilder getClassContentMeetingCancelEventBuilder(Object id, String classId) {
    return () -> new JsonObject().put(EVENT_NAME, EVT_CLASS_CONTENT_MEETING_CANCEL).put(EVENT_BODY,
        new JsonObject().put(CLASS_CONTENT_ID, id).put(ID_CLASS, classId));
  }
